...
describe('CategoryService', () => {
  ...
  it('can instantiate service with new', inject([Http], (http: Http) => {
    ...
    const service = new CategoryService(http);
  ...
  describe('when calling a service method', () => {
    ...
    it('getCategoryTree()', async(inject([], () => {
      const rootCat = new Category({id: 42, name: 'RooT'});
      const response = new Response(new ResponseOptions({status: 200, body: rootCat}));
    ...
    it('getCategoryTree()', async(inject([], () => {
      const categories = [new Category({id: 48, name: '48'}), new Category({id: 21, name: '21'})];
      const response = new Response(new ResponseOptions({status: 200, body: categories}));
      backend.connections.subscribe((c: MockConnection) => c.mockRespond(response));
    ...
    it('createCategory()', async(inject([], () => {
      const response = new Response(new ResponseOptions({status: 200, body: 777}));
      backend.connections.subscribe((c: MockConnection) => c.mockRespond(response));
    ...
    it('getSplitCategory()', async(inject([], () => {
      const splitCat = new Category({id: 21, name: 'SPLIT'});
      const response = new Response(new ResponseOptions({status: 200, body: splitCat}));
      backend.connections.subscribe((c: MockConnection) => c.mockRespond(response));
